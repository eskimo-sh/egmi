...
public class GlusterVolumeStopTest extends AbstractCommandTest {
    ...
    public void testCommand() throws Exception {
        ...
        GlusterVolumeStop command = new GlusterVolumeStop(mockClient, Volume.from("test_volume"));
        SimpleOperationResult result = command.execute(Node.from("127.0.0.1"), context);
        assertNotNull (result);
        assertTrue(result.isSuccess());

        assertEquals("127.0.0.1:12345/command?command=volume&subcommand=stop&options=test_volume%20--mode=script", url.get());
    }
