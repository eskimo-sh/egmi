...
public class ForceResetHostTest extends AbstractCommandTest {
    ...
    public void testCommand() throws Exception {
        ...
        ForceResetHost command = new ForceResetHost(
                mockClient);
        SimpleOperationResult result = command.execute(Node.from("127.0.0.1"), context);
        assertNotNull (result);
        assertTrue(result.isSuccess());

        assertEquals("127.0.0.1:12345/command?command=force-reset-host&subcommand=&options=", url.get());
    }
