...
public class RuntimeLayout {
    ...
    public RuntimeLayout (CommandContext context, Set<Node> activeNodes) {
        ...
        while (activeNodes.size() < targetNbrBricks) {
            context.info ("    - Should create " + targetNbrBricks + " brick but only " + activeNodes.size() + " nodes are up.");
            if (targetNbrShards > 1) {
                targetNbrShards--;
                targetNbrBricks = targetNbrShards * targetNbrReplicas;
            } else if (targetNbrReplicas > 1) {
                targetNbrReplicas--;
                targetNbrBricks = targetNbrShards * targetNbrReplicas;
            }
        }
