...
IF %ERRORLEVEL% NEQ 0 (
    if "%JAVA_HOME%"== "" (
        echo "No java in path neither JAVA_HOME defined. Impossible to launch EGMI"
        exit 1
    ) else (
...
FOR /f "tokens=*" %%G IN ('dir /b %scriptpath%..\lib\egmi*war') DO set WAR_FILE=%%G
...
%JAVA_HOME%\bin\java ^
    ...
    --spring.config.location=%scriptpath%..\conf\egmi.properties
